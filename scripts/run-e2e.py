...
def run_pipeline(hop_home: Path, pipeline: Path, env: dict[str, str], input_dir: Path, output_dir: Path) -> None:
    command = [
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="hop-geometry-calculator-e2e-") as temporary:
        ...
        run_pipeline(hop_home, ROOT / "e2e/geometry-calculator.hpl", env, input_dir, output_dir)
