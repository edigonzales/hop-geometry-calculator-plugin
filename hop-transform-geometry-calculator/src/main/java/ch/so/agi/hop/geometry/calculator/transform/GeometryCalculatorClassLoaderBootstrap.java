...
/** Initializes the shared Geometry Type classloader before pipeline transforms are loaded. */
@ExtensionPoint(
    id = "GeometryCalculatorClassLoaderBootstrap",
    extensionPointId = "HopEnvironmentAfterInit",
    description = "Initialize the shared Geometry Type classloader for Geometry Calculator")
public final class GeometryCalculatorClassLoaderBootstrap
  ...
  public void callExtensionPoint(
    ...
    if (geometryPlugin == null) {
      throw new HopException(
          "Geometry Calculator requires the hop-geometry-type plugin (ValueMeta plugin id "
              + GEOMETRY_VALUE_META_PLUGIN_ID
              + ")");
    }
    if (!GEOMETRY_CLASSLOADER_GROUP.equals(geometryPlugin.getClassLoaderGroup())) {
      throw new HopException(
          "Geometry Calculator requires hop-geometry-type to use classloader group '"
